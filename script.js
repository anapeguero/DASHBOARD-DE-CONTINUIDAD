...
const charts={};
let state={area:'Todas',quarter:'T1',page:'vp',storeSearch:'',supra:'',format:'',actionArea:'Todas',onlyOpen:true};
...
function investmentDetail(){
  ...
  return detail.filter(r=>{
    const storeMatch=!q||normalize([r.center,r.description].join(' ')).includes(q);
    const supraMatch=!state.supra||r.supra===state.supra;
    const formatMatch=!state.format||r.format===state.format;
    return storeMatch&&supraMatch&&formatMatch;
  });
}
...
function renderCenters(){
  ...
  if(state.supra)parts.push(`supranúmero: ${state.supra}`);
  if(state.format)parts.push(`formato: ${state.format}`);
  $('investmentFilterChip').textContent=parts.length?`Vista filtrada · ${parts.join(' · ')}`:'Vista: todas las tiendas · todos los supranúmeros · todos los formatos';
}
function renderDetail(){
  const rows=investmentDetail();
  $('detailCount').textContent=`${rows.length} registros`;
  $('detailBody').innerHTML=rows.slice(0,700).map(r=>`<tr>
    <td>${r.center}</td><td>${r.format}</td><td>${r.description}</td>
    <td class="num">${money(r.plan)}</td><td class="num">${money(r.real)}</td><td class="num">${money(r.committed)}</td>
    <td class="num">${money(r.real+r.committed)}</td><td class="num">${money(r.plan-r.real-r.committed)}</td>
    <td>${pct(r.plan?(r.real+r.committed)/r.plan:0)}</td><td>${r.area}</td>
  </tr>`).join('');
}
...
async function load(){
  ...
  const supras=[...new Set(detail.map(x=>x.supra).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es',{numeric:true}));
  const formats=[...new Set(detail.map(x=>x.format).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es'));
  $('supraSelect').innerHTML='<option value="">Todos los supranúmeros</option>'+supras.map(x=>`<option value="${x}">${x}</option>`).join('');
...
function bind(){
  document.querySelectorAll('.nav-btn').forEach(b=>b.onclick=()=>{
    ...
    $('headerSubtitle').textContent=state.page==='vp'?'VISIÓN EJECUTIVA':state.page==='general'?'EJECUCIÓN GENERAL':'ANÁLISIS DE INVERSIONES';
  ...
  $('supraSelect').onchange=e=>{state.supra=e.target.value;renderCenters();renderDetail()};
  $('formatSelect').onchange=e=>{state.format=e.target.value;renderCenters();renderDetail()};
  $('clearInvestmentFilters').onclick=()=>{
    state.storeSearch='';state.supra='';state.format='';
    $('storeSearch').value='';$('supraSelect').value='';$('formatSelect').value='';
