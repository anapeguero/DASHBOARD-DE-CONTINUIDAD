...
function populateFilters(){fillSelect("centerFilter",uniqueSorted("centro"));fillSelect("formatFilter",uniqueSorted("formato"));fillSupraFilter();fillSelect("statusFilter",uniqueSorted("status"))}
function applyFilters(){
  const center=$("centerFilter").value,format=$("formatFilter").value,supra=$("supraFilter").value,status=$("statusFilter").value;
  filteredRows=allRows.filter(r=>(!center||r.centro===center)&&(!format||r.formato===format)&&(!supra||r.supra===supra)&&(!status||r.status===status));render()
}
...
function renderStatusBucketChart(){
  destroyChart("statusBucket");
  const buckets=[
    {label:"Sin ejecutar",count:0},
    {label:"Baja ejecución (<50%)",count:0},
    {label:"En ejecución (50%-<100%)",count:0},
    {label:"Ejecutado (100%)",count:0},
    {label:"Sobre ejecutado (>100%)",count:0}
  ];
  filteredRows.forEach(r=>{
    const p=r.percent;
    if(p===0)buckets[0].count++;
    else if(p<0.5)buckets[1].count++;
    else if(p<1)buckets[2].count++;
    else if(Math.abs(p-1)<0.000001)buckets[3].count++;
    else buckets[4].count++;
  });
  charts.statusBucket=new Chart($("statusBucketChart"),{
    type:"bar",
    data:{
      labels:buckets.map(x=>x.label),
      datasets:[{label:"Partidas",data:buckets.map(x=>x.count),backgroundColor:["#8b989e","#ffd51f","#0aa4d6","#078d36","#d71920"]}]
    },
    options:{
      responsive:true,maintainAspectRatio:false,indexAxis:"y",
      plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>`${ctx.raw} partidas`}}},
      scales:{x:{beginAtZero:true,ticks:{precision:0},grid:{color:"#edf1f2"}},y:{grid:{display:false}}}
    }
  });
}
...
["centerFilter","formatFilter","supraFilter","statusFilter"].forEach(id=>$(id).addEventListener("change",applyFilters));
$("resetBtn").addEventListener("click",()=>{["centerFilter","formatFilter","supraFilter","statusFilter"].forEach(id=>$(id).value="");applyFilters()});
