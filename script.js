...
async function loadDefault(){
  ...
  catch(err){$("statusMessage").innerHTML=`<strong>No se pudo cargar el Excel automáticamente.</strong> ${escapeHtml(err.message)}. Puedes utilizar <b>Cargar otro Excel</b>.`;$("dataSource").textContent="Esperando archivo Excel..."}
}
...
function populateFilters(){fillSelect("centerFilter",uniqueSorted("centro"));fillSelect("formatFilter",uniqueSorted("formato"));fillSelect("descriptionFilter",uniqueSorted("descripcion"),"Todas");fillSelect("statusFilter",uniqueSorted("status"))}
function applyFilters(){
  const center=$("centerFilter").value,format=$("formatFilter").value,descripcion=$("descriptionFilter").value,status=$("statusFilter").value;
  filteredRows=allRows.filter(r=>(!center||r.centro===center)&&(!format||r.formato===format)&&(!descripcion||r.descripcion===descripcion)&&(!status||r.status===status));render()
}
...
function renderDescriptionChart(){destroyChart("description");const data=aggregate(filteredRows,"descripcion").slice(0,12);charts.description=new Chart($("descriptionChart"),{type:"bar",data:{labels:data.map(x=>shortLabel(x.label,32)),datasets:[{label:"Plan",data:data.map(x=>x.plan),backgroundColor:"#dce4e7"},{label:"Ejecución",data:data.map(x=>x.execution),backgroundColor:"#2525ad"}]},options:{...chartBase,indexAxis:"y",plugins:{legend:{display:true,position:"bottom",labels:{font:{size:10}}},tooltip:{callbacks:{title:items=>data[items[0].dataIndex].label}}},scales:{x:{ticks:{callback:v=>formatMillions(v)},grid:{color:"#edf1f2"}},y:{ticks:{font:{size:9}},grid:{display:false}}}}})}
function renderComposition(t){destroyChart("composition");charts.composition=new Chart($("compositionChart"),{type:"doughnut",data:{labels:["Real","Comprometido","Pendiente"],datasets:[{data:[Math.max(t.real,0),Math.max(t.committed,0),Math.max(t.pending,0)],backgroundColor:["#0aa4d6","#ffd51f","#dce4e7"],borderColor:"#fff",borderWidth:3}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:"bottom",labels:{font:{size:10},padding:12}}}}})}
function renderTable(){$("dataTable").innerHTML=filteredRows.map(r=>`<tr><td><b>${escapeHtml(r.centro)}</b></td><td>${escapeHtml(r.formato)}</td><td>${escapeHtml(r.descripcion)}</td><td class="num">${money2(r.plan)}</td><td class="num">${money2(r.real)}</td><td class="num">${money2(r.committed)}</td><td class="num"><b>${money2(r.execution)}</b></td><td class="num">${money2(r.pending)}</td><td class="num"><b>${pct(r.percent*100)}</b></td><td><span class="badge ${statusBadgeClass(r.status,r.percent)}">${escapeHtml(r.status||deriveStatus(r.percent))}</span></td></tr>`).join("")}
function deriveStatus(p){if(p>=1)return"Completado";if(p>0)return"En Progreso";return"Pendiente de ejecutar"}
function statusBadgeClass(status,p){const s=(status||"").toLowerCase();if(p>1||s.includes("sobre"))return"danger";if(p>=1||s.includes("complet"))return"done";if(s.includes("pendiente")||p===0)return"warn";return""}
...
["centerFilter","formatFilter","descriptionFilter","statusFilter"].forEach(id=>$(id).addEventListener("change",applyFilters));
$("resetBtn").addEventListener("click",()=>{["centerFilter","formatFilter","descriptionFilter","statusFilter"].forEach(id=>$(id).value="");applyFilters()});
$("fileInput").addEventListener("change",async e=>{const file=e.target.files[0];if(!file)return;try{await loadWorkbook(await file.arrayBuffer(),file.name);$("statusMessage").innerHTML=`<strong>Excel cargado:</strong> ${escapeHtml(file.name)}. Los indicadores y gráficos fueron recalculados usando <b>Real + Comprometido</b>.`}catch(err){$("statusMessage").innerHTML=`<strong>Error:</strong> ${escapeHtml(err.message)}`}});
