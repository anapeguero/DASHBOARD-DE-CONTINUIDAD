...
const moneyBlank=v=>Math.abs(Number(v)||0)<0.000001?'':money(v);
const num=v=>{const n=Number(v);return Number.isFinite(n)?n:0};
const txt=v=>(v??'').toString().trim();
const normalize=s=>txt(s).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();
const charts={};
let state={area:'Todas',quarter:'T1',page:'vp',investmentArea:'Todas',storeSearch:'',supraDesc:'',format:'',actionArea:'Todas',onlyOpen:true};
...
function vpDetail(){
  return detail.filter(r=>state.area==='Todas'||r.area===state.area);
}
function currentDetail(){
  // Ejecución General siempre trabaja con Operaciones + Mantenimiento en conjunto.
  return detail;
}
function investmentDetail(){
  const q=normalize(state.storeSearch);
  return detail.filter(r=>{
    const areaMatch=state.investmentArea==='Todas'||r.area===state.investmentArea;
    const storeMatch=!q||normalize([r.center,r.centerName||'',r.description].join(' ')).includes(q);
    const descMatch=!state.supraDesc||r.description===state.supraDesc;
    const formatMatch=!state.format||r.format===state.format;
    return areaMatch&&storeMatch&&descMatch&&formatMatch;
  });
}
...
function fillAnnual(prefix){
  const source=prefix==='vp'?vpDetail():currentDetail();
  const a=aggregate(source), e=execution(a), plan=a.plan;
  const rate=plan?e/plan:0, realRate=plan?a.real/plan:0, committedRate=plan?a.committed/plan:0;
  const pending=plan-e;
  const scaleMax=Math.max(plan,e,1);

  $(prefix+'Plan').textContent=money(plan);
  $(prefix+'Real').textContent=money(a.real);
  $(prefix+'Committed').textContent=money(a.committed);
  $(prefix+'Execution').textContent=money(e);
  $(prefix+'Pending').textContent=money(Math.max(0,pending));

  $(prefix+'PlanTop').textContent=money(plan);
  $(prefix+'ExecutionTop').textContent=money(e);
  $(prefix+'PlanInside').textContent=money(plan);
  $(prefix+'RealInside').textContent=money(a.real);
  $(prefix+'CommittedInside').textContent=money(a.committed);

  $(prefix+'AnnualPct').textContent=`${pct(rate)} de ejecución`;
  $(prefix+'RealPct').textContent=`${pct(realRate)} del plan`;
  $(prefix+'CommittedPct').textContent=`${pct(committedRate)} del plan`;
  $(prefix+'PendingPct').textContent=pending>=0?`${pct(plan?pending/plan:0)} del plan`:`${pct(plan?Math.abs(pending)/plan:0)} sobre plan`;

  const maxPx=245, planH=Math.max(8,(plan/scaleMax)*maxPx);
  const realH=Math.max(a.real?4:0,(a.real/scaleMax)*maxPx);
  const committedH=Math.max(a.committed?4:0,(a.committed/scaleMax)*maxPx);
  $(prefix+'PlanBar').style.height=`${planH}px`;
  $(prefix+'RealBar').style.height=`${realH}px`;
  $(prefix+'CommittedBar').style.height=`${committedH}px`;

  const gapLabel=$(prefix+'GapLabel'), gap=$(prefix+'Gap');
  const callout=$(prefix+'StatusCallout'), title=$(prefix+'StatusTitle'), cap=$(prefix+'AnnualCaption');
  if(pending>0){
    gapLabel.textContent='Por ejecutar';
    gap.textContent=money(pending);
    callout.className='annual-status-callout '+(rate>=.75?'good':'warn');
    title.textContent=rate>=.75?'Avance anual en línea':'Seguimiento requerido';
    cap.textContent=`Se ha cubierto ${pct(rate)} del plan anual mediante Real + Comprometido.`;
  }else if(pending<0){
    gapLabel.textContent='Sobre el plan';
    gap.textContent=money(Math.abs(pending));
    callout.className='annual-status-callout over';
    title.textContent='Ejecución sobre el plan';
    cap.textContent=`La ejecución supera el plan anual en ${money(Math.abs(pending))}.`;
  }else{
    gapLabel.textContent='Por ejecutar';
    gap.textContent=money(0);
    callout.className='annual-status-callout good';
    title.textContent='Meta anual cubierta';
    cap.textContent='La ejecución (Real + Comprometido) cubre el 100% del plan anual.';
  }
}
function quarterDef(){
  return [
...
function quarterCardsHtml(){
  return quarterDef().map(q=>{
    ...
    return `<article class="quarter-card">
      ...
      <div class="quarter-body">
        ...
        <div class="quarter-info">
          <p><span>Plan acumulado:</span><b>${money(a.plan)}</b></p>
          <p><span>Ejecución acumulada:</span><b>${money(a.executed)}</b></p>
...
function renderQuarterCards(){
  $('vpQuarterCards').innerHTML=quarterCardsHtml();
  if($('generalQuarterCards'))$('generalQuarterCards').innerHTML=quarterCardsHtml();
}
...
function renderCenters(){
  const rows=investmentDetail(), by=new Map();
  ...
  $('centerKpis').innerHTML=[
    ['Tiendas visibles',new Set(rows.map(r=>r.center)).size],['Plan',money(total.plan)],['Real',money(total.real)],['Comprometido',money(total.committed)],['% Ejecución',pct(total.plan?ex/total.plan:0)]
  ...
  const parts=[];
  if(state.investmentArea!=='Todas')parts.push(`área: ${state.investmentArea}`);
  if(state.storeSearch)parts.push(`tienda: ${state.storeSearch}`);
  if(state.supraDesc)parts.push(`descripción: ${state.supraDesc}`);
  if(state.format)parts.push(`formato: ${state.format}`);
  $('investmentFilterChip').textContent=parts.length?`Vista filtrada · ${parts.join(' · ')}`:'Vista: todas las áreas · todas las tiendas · todas las descripciones · todos los formatos';
}
function renderDetail(){
  const rows=investmentDetail();
  $('detailCount').textContent=`${rows.length} registros`;

  $('detailBody').innerHTML=rows.slice(0,700).map(r=>{
    const exec=r.real+r.committed;
    const pending=r.plan-exec;
    return `<tr>
      <td>${r.center}</td>
      <td>${r.format||''}</td>
      <td>${r.description||''}</td>
      <td class="num">${moneyBlank(r.plan)}</td>
      <td class="num">${moneyBlank(r.real)}</td>
      <td class="num">${moneyBlank(r.committed)}</td>
      <td class="num">${moneyBlank(exec)}</td>
      <td class="num">${moneyBlank(pending)}</td>
      <td>${r.area||''}</td>
    </tr>`;
  }).join('');
  ...
  const totals=rows.reduce((a,r)=>{
    ...
    return a;
  },{plan:0,real:0,committed:0});
  const totalExec=totals.real+totals.committed;
  const totalPending=totals.plan-totalExec;

  $('detailTotals').innerHTML=`<tr class="total-row">
    <td colspan="3"><b>TOTAL</b></td>
    <td class="num"><b>${moneyBlank(totals.plan)}</b></td>
    <td class="num"><b>${moneyBlank(totals.real)}</b></td>
    <td class="num"><b>${moneyBlank(totals.committed)}</b></td>
    <td class="num"><b>${moneyBlank(totalExec)}</b></td>
    <td class="num"><b>${moneyBlank(totalPending)}</b></td>
    <td></td>
  </tr>`;
}
function renderAll(){
...
async function load(){
  ...
  const supraDescriptions=[...new Set(detail.map(x=>x.description).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es',{numeric:true}));
  const formats=[...new Set(detail.map(x=>x.format).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'es'));
  $('supraDescSelect').innerHTML='<option value="">Todas las descripciones</option>'+supraDescriptions.map(x=>`<option value="${x}">${x}</option>`).join('');
  $('formatSelect').innerHTML='<option value="">Todos los formatos</option>'+formats.map(x=>`<option value="${x}">${x}</option>`).join('');
...
function bind(){
  document.querySelectorAll('.nav-btn').forEach(b=>b.onclick=()=>{
    ...
    $('headerSubtitle').textContent=state.page==='vp'?'VISIÓN EJECUTIVA':state.page==='general'?'EJECUCIÓN GENERAL':'EJECUCIÓN AL DETALLE';
    setTimeout(renderAll,30);
  });
  document.querySelectorAll('#areaFilter button').forEach(b=>b.onclick=()=>{
    state.area=b.dataset.area;
    document.querySelectorAll('#areaFilter button').forEach(x=>x.classList.toggle('active',x===b));
    updateViewChip();
    fillAnnual('vp');
    renderActions();
  });
  ...
  document.querySelectorAll('#investmentAreaFilter button').forEach(b=>b.onclick=()=>{
    state.investmentArea=b.dataset.investmentArea;
    document.querySelectorAll('#investmentAreaFilter button').forEach(x=>x.classList.toggle('active',x===b));
    renderCenters();renderDetail();
  });
  $('storeSearch').oninput=e=>{state.storeSearch=e.target.value;renderCenters();renderDetail()};
  $('supraDescSelect').onchange=e=>{state.supraDesc=e.target.value;renderCenters();renderDetail()};
  $('formatSelect').onchange=e=>{state.format=e.target.value;renderCenters();renderDetail()};
  $('clearInvestmentFilters').onclick=()=>{
    state.investmentArea='Todas';state.storeSearch='';state.supraDesc='';state.format='';
    document.querySelectorAll('#investmentAreaFilter button').forEach(x=>x.classList.toggle('active',x.dataset.investmentArea==='Todas'));
    $('storeSearch').value='';$('supraDescSelect').value='';$('formatSelect').value='';
    renderCenters();renderDetail();
  };
}
